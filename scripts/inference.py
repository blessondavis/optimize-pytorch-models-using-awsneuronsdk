...
import json 
import torch
# import tensorflow if any issue related to protobuf format. 
import torch.neuron
...
def model_fn(model_dir):
    """This function loads the model and the tokenizer from the model directory"""
...
def input_fn(serialized_input_data, content_type=JSON_CONTENT_TYPE):
    """Converts to the input json data and sends it to the predict_fn()"""
...
def predict_fn(input_data, models):
    """Loads the input data and model to make predictions. All custom logic before and after the prediction
     also be can be done in this part."""
...
def output_fn(prediction_output, accept=JSON_CONTENT_TYPE):
    """Converts the outputs of the predict_fn() in to json format"""
